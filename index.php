...
las palabras 'EDG'. /i ignora la diferencia de maiuscula y minuscula*/
 ...
 tipico de versiones. Números y puntos*/
        ...
        $sistemaOperativo = php_uname($navegador);
        return 'Tu navegador es el: EDGE.'."<br><br>".'La versión actual es: '.$versionActual. "<br><br>". 'Tu sistema operativo es: '.$sistemaOperativo;
    } else if (preg_match('/Chrome/i',$navegador)) {
        preg_match('/Chrome\/([\d\.]+)/i',$navegador,$version);
        $versionActual = $version[1];
        $sistemaOperativo = php_uname($navegador);
        return 'Tu navegador es el: Chrome.'."<br><br>".'La versión actual es: '.$versionActual. "<br><br>". 'Tu sistema operativo es: '.$sistemaOperativo;;
    } else if (preg_match('/Opera/i', $navegador) || preg_match('/OPR/i', $navegador)) {
        preg_match('/Opera\/([\d\.]+)/i',$navegador,$version);
        $versionActual = $version[1];
        $sistemaOperativo = php_uname($navegador);
        return 'Tu navegador es el: Opera.'."<br><br>".'La versión actual es: '.$versionActual. "<br><br>". 'Tu sistema operativo es: '.$sistemaOperativo;;
    } else if (preg_match('/Safari/i',$navegador)) {
        preg_match('/Safari\/([\d\.]+)/i',$navegador,$version);
        $versionActual = $version[1];
        $sistemaOperativo = php_uname($navegador);
        return 'Tu navegador es el: Safari.'."<br><br>".'La versión actual es: '.$versionActual. "<br><br>". 'Tu sistema operativo es: '.$sistemaOperativo;;
    } else if (preg_match('/Firefox/i', $navegador)) {
        preg_match('/Firefox\/([\d\.]+)/i',$navegador,$version);
        $versionActual = $version[1];
        $sistemaOperativo = php_uname($navegador);
        return 'Tu navegador es el: FireFox.'."<br><br>".'La versión actual es: '.$versionActual. "<br><br>". 'Tu sistema operativo es: '.$sistemaOperativo;;
    } else{
